...
class ProductController {
    ...
    public function index(Request $request): Response {
        ...
        $html = $this->view->render('products/index', compact('products','page','pages'));
        return new Response($html);
    }

    public function create(): Response {
        $html = $this->view->render('products/create', ['csrf' => Csrf::token(), 'errors' => []]);
        return new Response($html);
    }
    ...
    public function store(Request $request): Response {
        ...
        if ($errors) {
            $html = $this->view->render('products/create', ['csrf' => Csrf::token(), 'errors' => $errors, 'old' => $request->request->all()]);
        ...
        return new RedirectResponse('/products/show?id=' . $id);
    }
    ...
    public function show(Request $request): Response {
        ...
        $html = $this->view->render('products/show', ['product' => $product]);
        return new Response($html);
    }
    ...
    public function edit(Request $request): Response {
        ...
        $product = $this->repo->find($id);
        if (!$product) return new Response('Produto não encontrado', 404);
        $html = $this->view->render('products/edit', ['product' => $product, 'csrf' => Csrf::token(), 'errors' => []]);
        return new Response($html);
    }
    ...
    public function update(Request $request): Response {
        ...
        if ($errors) {
            $html = $this->view->render('products/edit', ['product' => array_merge($this->repo->find((int)$data['id']), $data), 'csrf' => Csrf::token(), 'errors' => $errors]);
            return new Response($html, 422);
        }
        ...
        return new RedirectResponse('/products/show?id=' . $product->id);
    }
